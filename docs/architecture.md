...
The schedule engine in each implementation reads `shared/schedule/bell-schedule.json`. The file contains regular, advisory, and first-day/all-periods block times, mini-block times, and all four lunch waves. The Windows build embeds it and copies it into publish output; the Linux build embeds it in the Qt resource system; the macOS packaging script copies it into `Contents/Resources`.
